...
var emergency_data_default = router;
...
var reminders = pgTable("reminders", {
  ...
  // daily, weekly, monthly, yearly, once, context_based
  date: timestamp("date"),
  userId: integer("user_id").references(() => users.id),
  featureId: integer("feature_id").references(() => features.id),
  completed: boolean("completed").default(false),
  metadata: json("metadata"),
  // For priority, contextTriggers, category, relatedDocuments
  createdAt: timestamp("created_at").defaultNow()
});
...
router3.get("/category/:id", async (req, res) => {
  try {
...
var subcategories_default = router3;
...
router4.get("/subcategory/:id", async (req, res) => {
  try {
...
router5.post("/", upload.single("file"), async (req, res) => {
  try {
    if (!req.file) {
...
router7.get("/feature/:id", async (req, res) => {
  try {
